...
log = logging.getLogger("red.wowlogs.http")
...
async def generate_bearer(bot: Red, config: Config) -> str:
    ...
    tokens = await bot.get_shared_api_tokens("wowlogs")
    ...
    if not client_id:
        ...
        headers = {
            "User-Agent": "Red-DiscordBot/WowLogsCog"
        }
        async with aiohttp.ClientSession(
            ...
            if json.get("error", ""):
                log.error("There is an error generating the bearer key, probally a misconfigured client id and secret.")
            ...
            bearer_timestamp = timestamp_now + json['expires_in'] - 120  # Reduce the timestamp by 2 min to be on the safe side of possible errors
            await config.bearer_timestamp.set(bearer_timestamp)

            log.info("Bearer token created")
            await bot.set_shared_api_tokens("wowlogs", bearer=json['access_token'])
